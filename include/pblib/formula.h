...
public:
  ...
  FormulaClass(int32_t flags, int32_t data, bool isCopyDummy)
      : flags(flags), data(data) {
    assert((flags & 2) != 0);  // Do NOT use this ctor for compound formulas!
                               // You would copy the id!
  ...
  FormulaClass(int32_t flags, int32_t data, std::vector<Formula>& input_nodes,
               bool isCopyDummy)
      : flags(flags), data(data), input_nodes(input_nodes) {
    assert((flags & 2) == 0);
    assert(flags != 0 || data == 0 || data == 1 || data == 4);
  };

  FormulaClass(int32_t flags, std::vector<Formula>& input_nodes,
               int32_t var = 0)
      : flags(flags), input_nodes(input_nodes) {
    init(var);
    assert(flags != 0 || data == 0 || data == 1 || data == 4);
  };
...
public:
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i)
          if (it->second[i]->data == (f->data ^ 1)) return it->second[i];
      }
    }

    Formula result =
        (isAtom(f))
            ? std::make_shared<FormulaClass>(f->flags, f->data ^ 1, true)
            : std::make_shared<FormulaClass>(f->flags, f->data ^ 1,
                                             f->input_nodes, true);

    if (config->use_formula_cache) formula_cache[hash].push_back(result);
    return result;
  }

  static Formula newAND(std::vector<Formula>& conjuncts) {
    assert(false &&
           "multiple AND nodes are currently depracted (since the result in "
           "fewer fomrula history hits)");  // multiple AND nodes are currently
                                            // depracted (since the result in
                                            // fewer fomrula history hits)
    if (config->use_formula_cache)
      assert(false && "if this is implemented again .. add code here");

    std::sort(conjuncts.begin(), conjuncts.end());
    assert(conjuncts.size() > 2);
    uint64_t hash = (((uint64_t)conjuncts[0]->data) << 32) ^
                    (((uint64_t)conjuncts[1]->data));
    ...
    if (it != formula_cache.end()) {
      for (int i = 0; i < it->second.size(); ++i) {
        if (isAND(it->second[i]) &&
            conjuncts.size() == it->second[i]->input_nodes.size()) {
          bool equal = true;
          ...
          if (equal == true) return it->second[i];
        }
      }
    }
  ...
  static Formula newAND(Formula const& f, Formula const& g) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i) {
          if (isAND(it->second[i]) && it->second[i]->input_nodes[0] == f &&
              it->second[i]->input_nodes[1] == g)
            return it->second[i];
        }
      }
    }
  ...
  static Formula newEquiv(Formula const& f, Formula const& g) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i) {
          if (isEquiv(it->second[i]) && it->second[i]->input_nodes[0] == f &&
              it->second[i]->input_nodes[1] == g)
            return it->second[i];
        }
      }
  ...
  static Formula newFAs(Formula const& x, Formula const& y, Formula const& c) {
    ...
    if (config->use_formula_cache) {
      ...
      hash = ((sd & 3) ^ (td & 3) ^ (fd & 3)) ^ (ha << 2) ^ (hb << 16) ^
             (hc << 32);

      it = formula_cache.find(hash);
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i) {
          if (isFAs(it->second[i]) && it->second[i]->input_nodes[0] == x &&
              it->second[i]->input_nodes[1] == y &&
              it->second[i]->input_nodes[2] == c)
            return it->second[i];
        }
      }
    }
  ...
  static Formula newFAc(Formula const& x, Formula const& y, Formula const& c) {
    ...
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i) {
          if (isFAc(it->second[i]) && it->second[i]->input_nodes[0] == x &&
              it->second[i]->input_nodes[1] == y &&
              it->second[i]->input_nodes[2] == c)
            return it->second[i];
        }
  ...
  static Formula newMonotonic_ITE(Formula const& s, Formula const& t,
                                  Formula const& f) {
    uint64_t hash;
    if (config->use_formula_cache) {
      ...
      if (it != formula_cache.end()) {
        for (int i = 0; i < it->second.size(); ++i) {
          if (isMonotonicITE(it->second[i]) && selector(it->second[i]) == s &&
              true_branch(it->second[i]) == t &&
              false_branch(it->second[i]) == f)
            return it->second[i];
        }
      }
    }
...
inline Lit getLit(Formula const& f) {
  assert(isAtom(f));
  return ((f->data & 1) != 0 ? -1 * (f->data >> 2) : (f->data >> 2));
}
inline Lit getId(Formula const& f) {
  assert(!isAtom(f));
  return ((f->data & 1) != 0 ? -1 * (f->data >> 2) : (f->data >> 2));
...
inline Formula noNeg(Formula const& f) { return FormulaClass::newNoNeg(f); }
...
inline Formula true_branch(Formula const& f) {
  assert(isITE(f) || isMonotonicITE(f));
  return f->input_nodes[1];
}
inline Formula false_branch(Formula const& f) {
  assert(isITE(f) || isMonotonicITE(f));
  return f->input_nodes[2];
}
inline Formula selector(Formula const& f) {
  assert(isITE(f) || isMonotonicITE(f));
  return f->input_nodes[0];
}
inline Formula left(Formula const& f) {
  assert(isAND(f) || isEquiv(f));
  return f->input_nodes[0];
}
inline Formula right(Formula const& f) {
  assert(isAND(f) || isEquiv(f));
...
// inline Formula realNeg(Formula  f) { assert(f.unique()); f->data = f->data ^
// 1; return f;}
