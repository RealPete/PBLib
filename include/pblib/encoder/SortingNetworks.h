...
private:
  ...
  void oddEvenMerge(std::vector<Formula>& fs, int begin, int end) {
    ...
    if (end - begin == 2)
      ...
      int mid = (end - begin) / 2;
      std::vector<Formula> tmp;
      for (int i = 0; i < end - begin; i++) tmp.push_back(fs[begin + i]);
      unriffle(tmp);
      oddEvenMerge(tmp, 0, mid);
      oddEvenMerge(tmp, mid, tmp.size());
      riffle(tmp);
      for (int i = 1; i < tmp.size() - 1; i += 2) cmp2(tmp, i);
      for (int i = 0; i < tmp.size(); i++) fs[i + begin] = tmp[i];
    }
  }
  ...
  void oddEvenSort(std::vector<Formula>& fs) {
    ...
    if (fs.size() < sz) fs.resize(sz, _false_);

    for (int i = 1; i < fs.size(); i *= 2)
      for (int j = 0; j + 2 * i <= fs.size(); j += 2 * i)
        oddEvenMerge(fs, j, j + 2 * i);

    for (int i = 0; i < sz - orig_sz; ++i) fs.pop_back();
  }

  void optimizeBase(std::vector<int64_t>& seq, int carry_ins,
                    std::vector<int64_t>& rhs, int cost, std::vector<int>& base,
                    int& cost_bestfound, std::vector<int>& base_bestfound) {
    if (cost >= cost_bestfound) return;

    // "Base case" -- don't split further, build sorting network for current
    // sequence:
    int64_t final_cost = 0;
    for (int i = 0; i < seq.size(); i++) {
      final_cost += seq[i];

      if (final_cost < 0) goto TooBig;
    }
  ...
  TooBig:;
    ...
    if (p != -1) {
#else
    // int upper_lim = (seq.size() == 0) ? 1 : seq.back(); // <<== Check that
    // sqRoot is an 'int' (no truncation of 'int64_t') for (int i = 0; i <
    // (int)elemsof(primes) && primes[i] <= upper_lim; i++){
    for (int i = 0; i < (int)primes.size(); i++) {
      ...
      for (int j = 0; j < rhs.size(); j++) {
        ...
        if (new_rhs.size() == 0 || div > new_rhs.back()) {
          ...
          if (!(rem == 0 && rest < p) && !(rem > rest)) digit_important = true;
        }
        /* <<==
        om 'rhs' slutar på 0:a och 'rest' inte kan overflowa, då behövs inte det
        sorterande nätverket för 'rest' ("always TRUE") samma sak om 'rhs' sista
        siffra är strikt större än 'rest' ("never TRUE")
        */
      }

      base.push_back(p);
      /**/ depth++;
      optimizeBase(new_seq, rest / p, new_rhs,
                   cost + (digit_important ? rest : 0), base, cost_bestfound,
                   base_bestfound);
      /**/ depth--;
      base.pop_back();
  ...
  void optimizeBase(std::vector<int64_t>& seq, std::vector<int64_t>& rhs,
                    int& cost_bestfound, std::vector<int>& base_bestfound) {
    std::vector<int> base;
...
#define lit2fml(p) id(var(var(p)), sign(p))
  ...
  void buildSorter(std::vector<Formula>& ps, std::vector<int>& Cs,
                   std::vector<Formula>& out_sorter) {
    out_sorter.clear();
    for (int i = 0; i < ps.size(); i++)
      for (int j = 0; j < Cs[i]; j++) out_sorter.push_back(ps[i]);
    oddEvenSort(out_sorter);  // (overwrites inputs)
  }

  void buildSorter(std::vector<Formula>& ps, std::vector<int64_t>& Cs,
                   std::vector<Formula>& out_sorter) {
    std::vector<int> Cs_copy;
    for (int i = 0; i < Cs.size(); i++) Cs_copy.push_back(Cs[i]);
    buildSorter(ps, Cs_copy, out_sorter);
  }

  class Exception_TooBig {};

  void buildConstraint(std::vector<Formula>& ps, std::vector<int64_t>& Cs,
                       std::vector<Formula>& carry, std::vector<int>& base,
                       int digit_no,
                       std::vector<std::vector<Formula> >& out_digits) {
    ...
    if (digit_no == base.size()) {
      ...
      for (int i = 0; i < carry.size(); i++)
        ps.push_back(carry[i]), Cs.push_back(1);
      ...
      // Add carry bits:
      for (int i = 0; i < carry.size(); i++)
        ps_rem.push_back(carry[i]), Cs_rem.push_back(1);
      ...
      // Get carry bits:
      carry.clear();
      for (int i = B - 1; i < result.size(); i += B) carry.push_back(result[i]);
      ...
      for (int i = 0; i < B - 1; i++) {
        ...
        for (int j = 0; j < result.size(); j += B) {
          int n = j + B - 1;
          if (j + i < result.size())
            out = OR(out, AND(result[j + i],
                              ((n >= result.size()) ? _true_ : ~result[n])));
        }
        out_digits.back().push_back(out);
      }

      buildConstraint(ps_div, Cs_div, carry, base, digit_no + 1,
                      out_digits);  // <<== change to normal loop
    }
  }

  /*
  Naming:
    - a 'base' is a vector of integers, stating how far you count at that
  position before you wrap to the next digit (generalize base).
    - A 'dig' is an integer representing a digit in a number of some base.
    - A 'digit' is a vector of formulas, where the number of 1:s represents a
  digit in a number of some base.
  */

  void convert(int64_t num, std::vector<int>& base,
               std::vector<int>& out_digs) {
  ...
  Formula lexComp(int sz, std::vector<int>& num,
    ...
    if (sz == 0)
      ...
      Formula gt =
          (digit.size() > dig)
              ? digit[dig]
              : _false_;  // This digit is greater than the "dig" of 'num'.
      Formula ge = (dig == 0) ? _true_
                              : (digit.size() > dig - 1)
                                    ? digit[dig - 1]
                                    : _false_;  // This digit is greater than or
                                                // equal to the "dig" of 'num'.
  ...
  Formula lexComp(std::vector<int>& num,
                  std::vector<std::vector<Formula> >& digits) {
    assert(num.size() == digits.size());
    return lexComp(num.size(), num, digits);
  }

  Formula buildConstraint(std::vector<Formula>& ps, std::vector<int64_t>& Cs,
                          std::vector<int>& base, int64_t lo, int64_t hi) {
    ...
    if (lo != INT_MIN) convert(lo, base, lo_digs);
    if (hi != INT_MAX)
      convert(hi + 1, base,
              hi_digs);  // (+1 because we will change '<= x' to '!(... >= x+1)'
    ...
    if (lo != Int_MIN){
        pf("lo=%d :", lo); for (int i = 0; i < lo_digs.size(); i++) pf(" %d",
    lo_digs[i]); pf("\n"); } if (hi != Int_MAX){ pf("hi+1=%d :", hi+1); for (int
    i = 0; i < hi_digs.size(); i++) pf(" %d", hi_digs[i]); pf("\n"); } END*/
