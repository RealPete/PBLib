...
//  {CNF} such that Generalized Arc Consistency is Maintained}, booktitle = {KI
//  2014: Advances in Artificial Intelligence}, series = {Lecture Notes in
//  Computer Science}, volume = {8736}, editor = {Carsten Lutz and Michael
//  Tielscher}, publisher={Springer Berlin Heidelberg}, pages={123--134},
//  language={English},
...
private:
  ...
  void binary_merge(const SimplePBConstraint& constraint,
                    ClauseDatabase& formula, AuxVarManager& auxvars,
                    int32_t gac_lit = 0);

  void totalizer(
      std::vector<int32_t> const& input, std::vector<int32_t>& output,
      ClauseDatabase& formula,
      AuxVarManager& auxvars);  // needed for the watchdog instantiation
  void unary_adder(
      std::vector<int32_t> const& a, std::vector<int32_t> const& b,
      std::vector<int32_t>& c, ClauseDatabase& formula,
      AuxVarManager& auxvars);  // needed for the watchdog instantiation
