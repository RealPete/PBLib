...
public:
  ...
  // add a Formula (gate) to the database (which is converted to clause, using
  // the addClause method)
  virtual void addFormula(Formula const& formula, AuxVarManager& aux_vars);

  // add the empty clause to database and mark hole formula as unsat (by setting
  // isUnsat to true and add empty clause with addClause) WARNING: be sure to
  // set isUnsat in you own implentation
