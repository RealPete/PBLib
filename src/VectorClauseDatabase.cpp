...
VectorClauseDatabase::~VectorClauseDatabase() {
  if (local_clauses) delete clauses;
}

VectorClauseDatabase::VectorClauseDatabase(PBConfig config)
    : ClauseDatabase(config),
      local_clauses(true),
      clauses(new vector<vector<int32_t> >()) {}

VectorClauseDatabase::VectorClauseDatabase(PBConfig config,
                                           vector<vector<int32_t> >* clauses)
    : ClauseDatabase(config), local_clauses(false), clauses(clauses) {}
