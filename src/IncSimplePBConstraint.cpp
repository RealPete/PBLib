...
IncSimplePBConstraint::IncSimplePBConstraint(
    int64_t max_sum, int64_t max_weight, int64_t normalizedOffset, PBTYPE type,
    vector<WeightedLit>& literals, Comparator comparator, int64_t less_eq,
    int64_t greater_eq)
    : SimplePBConstraint(max_sum, max_weight, type, literals, comparator,
                         less_eq, greater_eq) {
...
IncSimplePBConstraint::IncSimplePBConstraint(
    int64_t max_sum, int64_t max_weight, int64_t normalizedOffset, PBTYPE type,
    vector<WeightedLit>& literals, Comparator comparator, int64_t bound)
    : SimplePBConstraint(max_sum, max_weight, type, literals, comparator,
                         bound) {
  this->normalized_offset = normalizedOffset;
...
void IncSimplePBConstraint::encodeNewGeq(int64_t newGeq,
                                         ClauseDatabase& formula,
                                         AuxVarManager& auxVars) {
  assert(comparator == BOTH);
  geq = newGeq + normalized_offset;  // add the offset from the normalization to
                                     // the new bound
...
void IncSimplePBConstraint::encodeNewLeq(int64_t newLeq,
                                         ClauseDatabase& formula,
                                         AuxVarManager& auxVars) {
  leq = newLeq + normalized_offset;  // add the offset from the normalization to
                                     // the new bound
  if (leq >= init_leq) {
    leq = init_leq;
    return;
...
void IncSimplePBConstraint::setIncrementalData(
