...
void BinaryMerge::binary_merge(const SimplePBConstraint& constraint,
  ...
  int64_t m = ceil((double)(less_then) /
                   (double)(pow(2, p)));  // m is less (and not equal)
  ...
  for (int64_t i = 0; i <= p; ++i) {
    ...
    if ((T & bit) != 0) buckets[i].push_back(true_lit);

    for (int64_t j = 0; j < constraint.getN(); ++j) {
      if ((constraint.getWeightedLiterals()[j].weight & bit) != 0) {
        if (gac_lit != 0 &&
            constraint.getWeightedLiterals()[j].weight >= less_then) {
  ...
  vector<vector<int32_t> > bucket_card(p + 1);
  ...
  for (int i = 0; i < buckets.size(); ++i) {
    int64_t k =
        ceil((double)new_less_thean /
             (double)pow(
                 2, i));  // we need only k output bits to detect inconsistency
    ...
    if (k <= buckets[i].size()) {
      assert(k == bucket_card[i].size() ||
             config->use_watch_dog_encoding_in_binary_merger);
      if (gac_lit != 0) {
        formula.addConditionals(constraint.getConditionals());
    ...
    if (i > 0) {
      if (carries.size() > 0) {
        if (bucket_card[i].size() == 0) {
          ...
          if (config->use_watch_dog_encoding_in_binary_merger)
            unary_adder(bucket_card[i], carries, bucket_merge[i], formula,
                        auxvars);
          else
            Sorting::merge(k, bucket_card[i], carries, formula, auxvars,
                           bucket_merge[i]);

          if (k == bucket_merge[i].size() ||
              (config->use_watch_dog_encoding_in_binary_merger &&
               k <= bucket_merge[i].size())) {
            if (gac_lit != 0) {
              formula.addConditionals(constraint.getConditionals());
...
void BinaryMerge::encode(const SimplePBConstraint& pbconstraint,
  ...
  if (!config->use_gac_binary_merge)
    binary_merge(pbconstraint, formula,
                 auxvars);  // just check concistency with unit propagation
  else {
    // gac
    int32_t x;
    ...
    for (int i = 0; i < lits.size(); ++i) {
      if (config->binary_merge_no_support_for_single_bits &&
          floor(log2(lits[i].weight)) == log2(lits[i].weight)) {
      ...
      if (pbconstraint.getMaxWeight() == tmp.weight) {
        ...
        SimplePBConstraint constraint(pbconstraint.getMaxSum() - tmp.weight,
                                      maxWeight, PB, lits, LEQ,
                                      pbconstraint.getLeq() - tmp.weight);
        ...
        SimplePBConstraint constraint(pbconstraint.getMaxSum() - tmp.weight,
                                      pbconstraint.getMaxWeight(),
                                      pbconstraint.getType(), lits, LEQ,
                                      pbconstraint.getLeq() - tmp.weight);
        constraint.addConditionals(pbconstraint.getConditionals());
        if (constraint.getLeq() <= 0) {
          for (int j = 0; j < lits.size(); ++j) {
    ...
    if (config->binary_merge_no_support_for_single_bits &&
        encode_complete_constrainte)
      binary_merge(pbconstraint, formula, auxvars);
  }
}

BinaryMerge::BinaryMerge(PBConfig& config)
    : Encoder(config), old_card_encoder(config) {}

void BinaryMerge::totalizer(vector<int32_t> const& x, vector<int32_t>& u_x,
                            ClauseDatabase& formula, AuxVarManager& auxvars) {
  u_x.clear();
  if (x.size() == 0) return;
  ...
  if (x.size() == 1) {
    ...
    for (int i = 0; i < x.size(); ++i) u_x.push_back(auxvars.getVariable());

    vector<int32_t> x_1;
    vector<int32_t> x_2;
    int i = 0;
    for (; i < x.size() / 2; ++i) x_1.push_back(x[i]);
    for (; i < x.size(); ++i) x_2.push_back(x[i]);
...
void BinaryMerge::unary_adder(vector<int32_t> const& u,
                              vector<int32_t> const& v, vector<int32_t>& w,
                              ClauseDatabase& formula, AuxVarManager& auxvars) {
  w.clear();
  if (u.size() == 0) {
    for (int i = 0; i < v.size(); ++i) w.push_back(v[i]);
  } else if (v.size() == 0) {
    for (int i = 0; i < u.size(); ++i) w.push_back(u[i]);
  } else {
