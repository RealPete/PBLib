...
void Binary_AMO_Encoder::encode_intern(vector<int32_t>& literals,
...
int64_t Binary_AMO_Encoder::encodingValue(
    const SimplePBConstraint& pbconstraint) {
  int n = pbconstraint.getN();
  return valueFunction(
      n * ceil(log2(n)),
      ceil(log2(n)));  // from the paper .. seems to overestimating .. FIXME
}

void Binary_AMO_Encoder::encode(const SimplePBConstraint& pbconstraint,
                                ClauseDatabase& formula,
                                AuxVarManager& auxvars) {
  formula.addConditionals(pbconstraint.getConditionals());

  if (config->print_used_encodings) cout << "c encode with binary amo" << endl;
  ...
  k = (two_pow_nbits - _literals.size()) *
