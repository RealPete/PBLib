...
void AMO_Encoder::encode_intern(vector<Lit>& literals, ClauseDatabase& formula,
...
int64_t AMO_Encoder::clauseCount(int32_t n) {
  ...
  litCount++;
  return clauseCount(floor((double)n / 2) + 1) +
         clauseCount(ceil((double)n / 2) + 1);
}
...
void AMO_Encoder::encode(const SimplePBConstraint& pbconstraint,
...
void AMO_Encoder::encodeEq(const SimplePBConstraint& pbconstraint,
                           ClauseDatabase& formula, AuxVarManager& auxvars) {
  _literals.clear();
