...
// TODO implement an iterative (non-recursive) encoding  ... is this necessary?
// TODO performance check
void k_Product::encode_intern(vector<Lit>& literals, ClauseDatabase& formula,
                              AuxVarManager& auxvars) {
  if (literals.size() == 1) return;

  if (literals.size() == 2) {
    if (literals[0] == -literals[1]) return;
  }
...
void k_Product::encode_non_recursive(vector<Lit>& literals,
...
void k_Product::encode(const SimplePBConstraint& pbconstraint,
...
k_Product::k_Product(PBConfig& config)
