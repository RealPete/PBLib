...
void AdderEncoding::AdderIncData::encodeNewLeq(int64_t newLeq,
                                               ClauseDatabase& formula,
                                               AuxVarManager& auxVars,
                                               vector<int32_t> conditionals) {
  formula.addConditionals(conditionals);
...
void AdderEncoding::adderTree(vector<queue<int32_t> >& buckets,
...
void AdderEncoding::lessThanOrEqual(vector<int32_t>& xs, vector<int32_t>& ys,
                                    ClauseDatabase& formula) {
  assert(xs.size() == ys.size());
  vector<Lit> clause;
  bool skip;
  for (int i = 0; i < xs.size(); ++i) {
    if (ys[i] == 1 || xs[i] == 0) continue;
...
void AdderEncoding::numToBits(vector<int32_t>& bits, int64_t n,
                              int64_t number) {
  bits.clear();
...
// Generates units for "result == kBits", assuming kBits of them has only
// constant signals (0 or 1). result and kBits must have the same size
void AdderEncoding::resultIsEqual(vector<int32_t>& result,
  ...
  for (int i = 0; i < result.size(); ++i) {
    if (kBits[i] == 1) {
      ...
      assert(kBits[i] == 0);
      if (result[i] !=
          0)  // if result[i] == 0 -> -0 = true -> we do not have to add this
        formula->addClause(-result[i], 0);
    }
  }
}

void AdderEncoding::encode(
...
void AdderEncoding::encode(const SimplePBConstraint& pbconstraint,
  ...
  for (int iBit = 0; iBit < nb; ++iBit) {
    buckets.push_back(queue<int32_t>());
    result.push_back(0);
    for (int iVar = 0; iVar < pbconstraint.getWeightedLiterals().size();
         ++iVar) {
      if (((((int64_t)1) << iBit) &
           pbconstraint.getWeightedLiterals()[iVar].weight) != 0)
...
int64_t AdderEncoding::encodingValue(
    const shared_ptr<IncSimplePBConstraint>& pbconstraint) {
  return config->MAX_CLAUSES_PER_CONSTRAINT -
         1;  // since adder encoding is not GAC we use this as fallback only
}
