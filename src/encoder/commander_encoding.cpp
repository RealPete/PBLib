...
int64_t commander_encoding::encodingValue(
...
void commander_encoding::encode_non_recursive(vector<Lit>& literals,
...
void commander_encoding::encode_intern(vector<Lit>& literals,
                                       ClauseDatabase& formula,
                                       AuxVarManager& auxvars,
                                       bool isExactlyOne) {
  int i;
  ...
  while (current_literals.size() > k) {
    ...
    for (i = 0; i < current_literals.size(); ++i) {
      ...
      if (i % k == (k - 1) ||
          (i == current_literals.size() - 1))  // is group full?
      {
        ...
        if (isExactlyOne) {
          // 2. if the commander variable of a group is true, then at least one
          // of the variables in the group must be true (only for the exactly
          // one case)
          formula.addClause(_literals);  // at least one
        }

        // 3. if the commander variable of a group is false, then none of the
        // variables in the group can be true
...
void commander_encoding::encode(const SimplePBConstraint& pbconstraint,
                                ClauseDatabase& formula,
                                AuxVarManager& auxvars) {
  ...
  encode_intern(current_literals, formula, auxvars,
                (pbconstraint.getComparator() == PBLib::BOTH &&
...
commander_encoding::commander_encoding(PBConfig& config)
