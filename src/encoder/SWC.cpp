...
void SWC_Encoder::encode(const shared_ptr<IncSimplePBConstraint>& pbconstraint,
                         ClauseDatabase& formula, AuxVarManager& auxvars) {
...
void SWC_Encoder::encode_intern(const SimplePBConstraint& pbconstraint,
                                ClauseDatabase& formula, AuxVarManager& auxvars,
                                bool encodeComplete) {
  outlits.clear();
  if (pbconstraint.getComparator() == BOTH || isInc) encodeComplete = true;
  ...
  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= k; ++j) s[i][j] = auxvars.getVariable();

  for (int i = 2; i <= n; ++i)
    for (int j = 1; j <= k; ++j) formula.addClause(-s[i - 1][j], s[i][j]);

  for (int i = 1; i <= n; ++i)
    for (int j = 1; j <= lits[i - 1].weight; ++j)
      formula.addClause(-lits[i - 1].lit, s[i][j]);

  for (int i = 2; i <= n; ++i)
    for (int j = 1; j <= (k - lits[i - 1].weight); ++j)
      formula.addClause(-s[i - 1][j], -lits[i - 1].lit,
                        s[i][j + lits[i - 1].weight]);
  ...
  if (encodeComplete) {
    ...
    for (int i = 2; i <= (int)n; ++i)
      for (int j = lits[i - 1].weight + 1; j <= k; ++j)
        formula.addClause(-s[i][j], s[i - 1][j],
                          s[i - 1][j - lits[i - 1].weight]);

    for (int j = 1; j <= lits[0].weight; ++j)
      formula.addClause(-s[1][j], lits[0].lit);
  }

  for (int j = lits[0].weight + 1; j <= k; ++j) formula.addClause(-s[1][j]);
  ...
  if (pbconstraint.getComparator() == BOTH) {
    ...
    for (int j = 1; j <= pbconstraint.getGeq(); ++j) formula.addClause(s[n][j]);
  }
...
void SWC_Encoder::SWCIncData::encodeNewGeq(int64_t newGeq,
  ...
  if (newGeq > 0)
    formula.addClause(
        outlits[newGeq - 1]);  // setting newGeq to true forcing the sum to be
                               // at least equal newGeq

  for (int i = 0; i < conditionals.size(); ++i)
    formula.getConditionals().pop_back();
}

void SWC_Encoder::SWCIncData::encodeNewLeq(int64_t newLeq,
                                           ClauseDatabase& formula,
                                           AuxVarManager& auxVars,
                                           vector<int32_t> conditionals) {
  formula.addConditionals(conditionals);
  ...
  formula.addClause(-outlits[newLeq]);  // setting newLeq + 1 to false forcing
                                        // the sum to be less equal newLeq
...
int64_t SWC_Encoder::encodingValue(const SimplePBConstraint& pbconstraint) {
  ...
  return valueFunction(n * k * (pbconstraint.getComparator() == BOTH ? 4 : 2),
                       n * k);
}

int64_t SWC_Encoder::encodingValue(
    const shared_ptr<IncSimplePBConstraint>& pbconstraint) {
  int n = pbconstraint->getN();
  int64_t k = pbconstraint->getLeq();
