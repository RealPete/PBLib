...
void CardEncoding::CardIncData::encodeNewLeq(int64_t newLeq,
                                             ClauseDatabase& formula,
                                             AuxVarManager& auxVars,
                                             vector<int32_t> conditionals) {
  formula.addConditionals(conditionals);
...
int64_t CardEncoding::encodingValue(
...
void CardEncoding::encode(const shared_ptr<IncSimplePBConstraint>& pbconstraint,
  ...
  for (auto l : pbconstraint->getWeightedLiterals()) input.push_back(l.lit);

  if (pbconstraint->getComparator() == BOTH)
    Sorting::sort(leq + 1, input, formula, auxvars, output, Sorting::BOTH);
  else
    Sorting::sort(leq + 1, input, formula, auxvars, output,
                  Sorting::INPUT_TO_OUTPUT);

  formula.addConditionals(pbconstraint->getConditionals());

  if (output.size() >
      leq)  // if not, the constraint is always satisfiable (initially)
    formula.addClause(-output[leq]);  // setting k + 1 to false forcing the sum
                                      // to be less equal k
...
void CardEncoding::encode(const SimplePBConstraint& pbconstraint,
                          ClauseDatabase& formula, AuxVarManager& auxvars) {
  if (config->print_used_encodings) cout << "c encode with card" << endl;
  ...
  if (pbconstraint.getComparator() == LEQ && leq > (pbconstraint.getN() / 2)) {
    ...
    Sorting::sort(geq, input, formula, auxvars, output,
                  Sorting::OUTPUT_TO_INPUT);

    formula.addConditionals(pbconstraint.getConditionals());

    for (int i = 0; i < geq; ++i) formula.addClause(output[i]);
  ...
  formula.addClause(-output[leq]);  // setting k + 1 to false forcing the sum to
                                    // be less equal k
