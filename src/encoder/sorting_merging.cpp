#include <pblib/encoder/sorting_merging.h>
...
map<tuple<int32_t, int32_t, int32_t>, int64_t>
    Sorting::recursive_sorter_l_values;
map<tuple<int32_t, int32_t, int32_t>, int64_t> Sorting::recursive_merger_values;

void Sorting::recursive_sorter(int m, int l, const vector<int32_t>& input,
  ...
  for (int i = 0; i < l; ++i) tmp_lits_a.push_back(input[i]);

  for (int i = l; i < n; ++i) tmp_lits_b.push_back(input[i]);
...
void Sorting::recursive_sorter(int m, const vector<int32_t>& input,
                               ClauseDatabase& formula, AuxVarManager& auxvars,
                               vector<int32_t>& output,
                               ImplicationDirection direction) {
  assert(m > 0);
...
void Sorting::counter_sorter(int k, const vector<int32_t>& x,
                             ClauseDatabase& formula, AuxVarManager& auxvars,
                             vector<int32_t>& output,
                             ImplicationDirection direction) {
  int n = x.size();
  ...
  if (direction == INPUT_TO_OUTPUT || direction == BOTH) {
    for (int i = 0; i < n; ++i) {
      ...
      if (i > 0) formula.addClause(-s[i - 1][0], s[i][0]);
    }

    for (int j = 1; j < k; ++j) {
      for (int i = j; i < n; ++i) {
        formula.addClause(-x[i], -s[i - 1][j - 1], s[i][j]);
        if (i > j) formula.addClause(-s[i - 1][j], s[i][j]);
      }
    }
  }
  ...
  for (int i = 0; i < k; ++i) output.push_back(s[n - 1][i]);
}

void Sorting::direct_sorter(int m, const vector<int32_t>& input,
                            ClauseDatabase& formula, AuxVarManager& auxvars,
                            vector<int32_t>& output,
                            ImplicationDirection direction) {
  assert(direction == INPUT_TO_OUTPUT);
  ...
  output.clear();
  for (int i = 0; i < m; ++i) output.push_back(auxvars.getVariable());
...
void Sorting::sort(int m, const vector<int32_t>& input, ClauseDatabase& formula,
                   AuxVarManager& auxvars, vector<int32_t>& output,
                   ImplicationDirection direction) {
  assert(m >= 0);
  if (m == 0) {
    output.clear();
...
void Sorting::recursive_merger(int c, vector<int32_t> const& input_a, int a,
                               vector<int32_t> const& input_b, int b,
                               ClauseDatabase& formula, AuxVarManager& auxvars,
                               vector<int32_t>& output,
                               ImplicationDirection direction) {
  ...
  for (int i = 0; i < a; i = i + 2) tmp_lits_odd_a.push_back(input_a[i]);
  for (int i = 0; i < b; i = i + 2) tmp_lits_odd_b.push_back(input_b[i]);

  for (int i = 1; i < a; i = i + 2) tmp_lits_even_a.push_back(input_a[i]);
  for (int i = 1; i < b; i = i + 2) tmp_lits_even_b.push_back(input_b[i]);

  merge(c / 2 + 1, tmp_lits_odd_a, tmp_lits_odd_b, formula, auxvars, odd_merge,
        direction);
  merge(c / 2, tmp_lits_even_a, tmp_lits_even_b, formula, auxvars, even_merge,
        direction);
  ...
  while (true) {
    if (i < odd_merge.size() && j < even_merge.size()) {
      ...
      break;  // should never occure since we catch: if (output.size() == c)
              // break; (with c at most n)
...
void Sorting::direct_merger(int m, const vector<int32_t>& input_a,
                            const vector<int32_t>& input_b,
                            ClauseDatabase& formula, AuxVarManager& auxvars,
                            vector<int32_t>& output,
                            ImplicationDirection direction) {
  assert(direction == INPUT_TO_OUTPUT);
...
void Sorting::merge(int m, const vector<int32_t>& input_a,
                    const vector<int32_t>& input_b, ClauseDatabase& formula,
                    AuxVarManager& auxvars, vector<int32_t>& output,
                    PBLib::Sorting::ImplicationDirection direction) {
  assert(m >= 0);
  if (m == 0) {
    output.clear();
    return;
  ...
  if (direction != INPUT_TO_OUTPUT) {
    recursive_merger(m, input_a, input_a.size(), input_b, input_b.size(),
                     formula, auxvars, output, direction);
    return;
  }
...
void Sorting::comparator(const int32_t x1, const int32_t x2, const int32_t y,
...
void Sorting::comparator(const int32_t x1, const int32_t x2, const int32_t y1,
                         const int32_t y2, ClauseDatabase& formula,
                         ImplicationDirection direction) {
  assert(x1 != x2);
  assert(y1 != y2);
...
int64_t Sorting::direct_merger_value(int m, int a, int b,
                                     ImplicationDirection direction) {
  return value_function(
      (a + b) * m - (m * m - m) / 2 - (a * a - a) / 2 - (b * b - b) / 2, m);
}

int64_t Sorting::recursive_sorter_value(int m, int n, int l,
                                        ImplicationDirection direction) {
  auto entry =
      recursive_sorter_l_values.find(tuple<int32_t, int32_t, int32_t>(m, n, l));

  if (entry != recursive_sorter_l_values.end()) return entry->second;
  ...
  int64_t value = value_function(formula.getNumberOfClauses(),
                                 auxvars.getBiggestReturnedAuxVar() - n);
  recursive_sorter_l_values[tuple<int32_t, int32_t, int32_t>(m, n, l)] = value;

  return value;
}

int64_t Sorting::recursive_merger_value(int m, int a, int b,
                                        ImplicationDirection direction) {
  auto entry =
      recursive_merger_values.find(tuple<int32_t, int32_t, int32_t>(m, a, b));

  if (entry != recursive_merger_values.end()) return entry->second;
  ...
  for (int i = 0; i < a; ++i) input_a.push_back(i + 1);

  for (int i = 0; i < b; ++i) input_b.push_back(i + a + 1);

  recursive_merger(m, input_a, a, input_b, b, formula, auxvars, output,
                   direction);

  int64_t value = value_function(formula.getNumberOfClauses(),
                                 auxvars.getBiggestReturnedAuxVar() - a - b);
  recursive_merger_values[tuple<int32_t, int32_t, int32_t>(m, a, b)] = value;

  return value;
}

int64_t Sorting::recursive_sorter_value(int m, int n,
                                        ImplicationDirection direction) {
  auto entry = recursive_sorter_values.find(pair<int32_t, int32_t>(m, n));

  if (entry != recursive_sorter_values.end()) return entry->second;
...
int64_t Sorting::counter_sorter_value(int m, int n,
                                      ImplicationDirection direction) {
  return value_function(2 * n + (m - 1) * (2 * (n - 1) - 1) - (m - 2) -
                            2 * ((m - 1) * (m - 2) / 2),
                        m * n - m * (m - 1) / 2);
}

int64_t Sorting::direct_sorter_value(int m, int n,
                                     ImplicationDirection direction) {
  if (n > 30) return LLONG_MAX;
