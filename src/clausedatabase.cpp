...
void ClauseDatabase::addFormula(const Formula& formula,
                                AuxVarManager& aux_vars) {
...
void ClauseDatabase::deleteIsSetToUnsatFlag() { isUnsat = false; }

bool ClauseDatabase::isSetToUnsat() { return isUnsat; }
...
int32_t ClauseDatabase::polarityClausify(const Formula& f,
                                         AuxVarManager& aux_vars) {
  Lit result = 0;
    ...
    if ((f == _true_) || (f == _false_)) {
      ...
      if (f->input_nodes.size() == 2) {
        if (!isNeg(f)) {
          ...
          addClause(result, polarityClausify(~left(f), aux_vars),
                    polarityClausify(~right(f), aux_vars));
        }
      } else {
...
CountingClauseDatabase::CountingClauseDatabase(PBConfig config)
    : ClauseDatabase(config), number_of_clauses(0) {}

void CountingClauseDatabase::addClauseIntern(vector<int32_t> const& clause) {
  number_of_clauses++;
}

void CountingClauseDatabase::clearDataBase() { number_of_clauses = 0; }
