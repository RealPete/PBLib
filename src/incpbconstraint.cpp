...
int IncPBConstraint::getN() const { return weighted_literals.size(); }
...
int64_t IncPBConstraint::getGeq() const { return geq; }

int64_t IncPBConstraint::getLeq() const { return leq; }

vector<WeightedLit> const& IncPBConstraint::getWeightedLiterals() const {
  return weighted_literals;
}

void IncPBConstraint::setIncSimplePBConstraint(
...
void IncPBConstraint::setGeqIncSimplePBConstraint(
    shared_ptr<IncSimplePBConstraint> geqIncSimplePBConstraint) {
  isDualEncoded = true;
  geq_inc_simple_pb_constraint = geqIncSimplePBConstraint;
}

void IncPBConstraint::setLeqIncSimplePBConstraint(
    shared_ptr<IncSimplePBConstraint> leqIncSimplePBConstraint) {
  isDualEncoded = true;
  leq_inc_simple_pb_constraint = leqIncSimplePBConstraint;
}

void IncPBConstraint::encodeNewGeq(int64_t newGeq, ClauseDatabase& formula,
                                   AuxVarManager& auxVars) {
  if (newGeq <= init_geq) return;
  ...
  if (isDualEncoded) {
    ...
    geq_inc_simple_pb_constraint->encodeNewLeq(
        -geq, formula, auxVars);  // we encode GEQ as LEQ internaly
  } else {
    if (comparator == BOTH)
      inc_simple_pb_constraint->encodeNewGeq(geq, formula, auxVars);
    else {
      assert(comparator == GEQ);
      inc_simple_pb_constraint->encodeNewLeq(
          -geq, formula, auxVars);  // we encode GEQ as LEQ internaly
    }
  }
}

void IncPBConstraint::encodeNewLeq(int64_t newLeq, ClauseDatabase& formula,
                                   AuxVarManager& auxVars) {
  if (newLeq >= init_leq) return;
...
void IncPBConstraint::clearConditionals() { conditionals.clear(); }

const vector<int32_t>& IncPBConstraint::getConditionals() const {
  return conditionals;
}

void IncPBConstraint::setComparator(Comparator comp) { comparator = comp; }
