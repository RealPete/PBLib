...
void AuxVarManager::freeVariables(int32_t start, int32_t end) {
  for (; start <= end; ++start) freeVariable(start);
}

void AuxVarManager::freeVariables(vector<int32_t>& variables) {
  for (int i = 0; i < variables.size(); ++i) freeVariable(variables[i]);
}

void AuxVarManager::freeVariable(int32_t var) { free_variables.insert(var); }
...
int32_t AuxVarManager::getVariable() {
  ...
  if (free_variables.size() == 0) {
    var = variable_offset;
    variable_offset++;
    if (rememberedVariables != nullptr) rememberedVariables->push_back(var);
