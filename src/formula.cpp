...
Formula FormulaClass::newITE(const Formula& s, const Formula& t,
                             const Formula& f) {
  uint64_t hash;
  if (config->use_formula_cache) {
    ...
    hash =
        ((sd & 3) ^ (td & 3) ^ (fd & 3)) ^ (ha << 2) ^ (hb << 16) ^ (hc << 32);

    it = formula_cache.find(hash);
    if (it != formula_cache.end()) {
      for (int i = 0; i < it->second.size(); ++i) {
        if (isITE(it->second[i]) && selector(it->second[i]) == s &&
            true_branch(it->second[i]) == t && false_branch(it->second[i]) == f)
          return it->second[i];
      }
    }
  }
...
Formula operator~(Formula const& f) {
  if (f == _true_) return _false_;

  if (f == _false_) return _true_;

  if (f == _undef_) return _undef_;
...
Formula AND(std::vector<Formula>& conjuncts) {
  for (int i = 0; i < conjuncts.size(); ++i) {
    if (conjuncts[i] == _false_) return _false_;
