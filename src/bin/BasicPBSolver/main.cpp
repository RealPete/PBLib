...
int readValueVector(vector<int32_t>& valueVector, int64_t& ovalue,
                    bool& hasOValue) {
  valueVector.clear();
  ...
  while (getline(file, line)) {
    ...
    if (line[0] == 'v') {
      ...
      while (line.size() > ind) {
        ...
        if (number >= valueVector.size()) valueVector.resize(number + 1, 0);
...
bool check_constraints(vector<int32_t> const& model,
                       vector<PBConstraint> const& constraints) {
  for (PBConstraint const& constraint : constraints) {
    ...
    for (WeightedLit lit : constraint.getWeightedLiterals()) {
      ...
      if (model[abs(lit.lit)] == lit.lit) sum += lit.weight;
    }

    if (constraint.getComparator() == LEQ ||
        constraint.getComparator() == BOTH) {
...
int64_t getSumFromModel(P& constraint, vector<int32_t> const& model) {
  ...
  for (int i = 0; i < constraint.getN(); ++i) {
    ...
    if (model[abs(constraint.getWeightedLiterals()[i].lit)] *
            (constraint.getWeightedLiterals()[i].lit > 0 ? 1 : -1) >
        0)
...
void printModel(vector<int32_t>& model, int number_of_vars) {
  ...
  if (model.size() <= number_of_vars) {
    cout << "c error: cant print model since model size is smaller then the "
            "number of variables"
         << endl;
    return;
  }
...
void basicSearch(bool cnf_output_only, double tstart,
                 vector<PBConstraint>& constraints, PBParser& parser,
                 PBConfig config) {
  statistic stats;
  ...
  cout << "c start encoding ... ";
  cout.flush();
  if (config->print_used_encodings) cout << endl;
  ...
  tend = clock();
  cout << "done (parsing and encoding: " << (tend - tstart) / CLOCKS_PER_SEC
       << " sec )" << endl;
  ...
  if (!result) {
    ...
    if (config->cmd_line_options.find("model") !=
            config->cmd_line_options.end() ||
        config->cmd_line_options.find("allmodels") !=
            config->cmd_line_options.end()) {
      vector<int32_t> model;
      satsolver->getModel(model);
  ...
  while (true) {
    ...
    if (config->cmd_line_options.find("allmodels") !=
        config->cmd_line_options.end()) {
      printModel(model, parser.getMaxVarID());
    }
...
int main(int argc, char** argv) {
  ...
  if (argc > 2) {
    ...
    for (int i = 2; i < argc; ++i) {
      ...
      options[tmp.substr(0, tmp.find("="))] =
          tmp.find("=") == string::npos ? "" : tmp.substr(tmp.find("=") + 1);
    }
  }

  if (config->cmd_line_options.find("valid") !=
      config->cmd_line_options.end()) {
    vector<int32_t> model;
    int64_t o_value = 0;
  ...
  if (options.count("maxclauses") > 0) {
    ...
    cout << "c setting max clauses per constraint to " << options["maxclauses"]
         << endl;
  }
  ...
  if (config->cmd_line_options.find("norobdds") !=
      config->cmd_line_options.end())
    config->use_real_robdds = false;

  if (config->cmd_line_options.find("rectest") !=
      config->cmd_line_options.end())
    config->use_recursive_bdd_test = true;

  if (config->cmd_line_options.find("itertest") !=
      config->cmd_line_options.end())
  ...
  if (config->cmd_line_options.find("amo_bdd") !=
      config->cmd_line_options.end())
    config->amo_encoder = AMO_ENCODER::BDD;

  if (config->cmd_line_options.find("bimander") !=
      config->cmd_line_options.end())
    config->amo_encoder = AMO_ENCODER::BIMANDER;

  if (config->cmd_line_options.find("commander") !=
      config->cmd_line_options.end())
    config->amo_encoder = AMO_ENCODER::COMMANDER;

  if (config->cmd_line_options.find("kproduct") !=
      config->cmd_line_options.end())
  ...
  if (config->cmd_line_options.find("pairwise") !=
      config->cmd_line_options.end())
    config->amo_encoder = AMO_ENCODER::PAIRWISE;

  if (config->cmd_line_options.find("amk_bdd") !=
      config->cmd_line_options.end())
  ...
  if (config->cmd_line_options.find("pb_adder") !=
      config->cmd_line_options.end())
    config->pb_encoder = PB_ENCODER::ADDER;

  if (config->cmd_line_options.find("pb_sorter") !=
      config->cmd_line_options.end())
    config->pb_encoder = PB_ENCODER::SORTINGNETWORKS;

  if (config->cmd_line_options.find("watchdog") !=
      config->cmd_line_options.end()) {
    config->pb_encoder = PB_ENCODER::BINARY_MERGE;
    config->use_watch_dog_encoding_in_binary_merger = true;
  }

  if (config->cmd_line_options.find("bin_merge") !=
      config->cmd_line_options.end()) {
    config->pb_encoder = PB_ENCODER::BINARY_MERGE;
    config->use_gac_binary_merge = true;
  }

  if (config->cmd_line_options.find("bin_merge_no_gac") !=
      config->cmd_line_options.end())
    config->use_gac_binary_merge = false;

  if (config->cmd_line_options.find("watchdog_no_gac") !=
      config->cmd_line_options.end())
    config->use_gac_binary_merge = false;

  if (config->cmd_line_options.find("bin_merge_all_support") !=
      config->cmd_line_options.end())
    config->binary_merge_no_support_for_single_bits = false;

  if (config->cmd_line_options.find("non_gac_bdds") !=
      config->cmd_line_options.end())
  ...
  if (config->cmd_line_options.find("analyse") !=
      config->cmd_line_options.end()) {
    analyse(argv);
    return 0;
  }
  ...
  cout << "c Memory usage: " << (size_t)(rusage.ru_maxrss / 1024L) << " mbyte"
       << endl;
