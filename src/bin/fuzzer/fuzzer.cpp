...
void testNormalConstraints(PBConfig config, PBFuzzer& fuzzer,
                           int64_t rounds = -1);
void testConditionalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                int64_t rounds = -1);
void testIncrementalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                int64_t rounds = -1);
void testConditionalIncrementalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                           int64_t rounds = -1);
...
int main(int argc, char** argv) {
  ...
  if (options.find("randCalls") != options.end()) {
    ...
    for (int i = 0; i < randCalls; ++i) rand();
  ...
  while (rounds < 0 ? true : (count < rounds)) {
    ...
    for (auto config : configs) {
      ...
      cout << "c rand() counts: " << RandomCounter::callCount
           << "                                              " << endl;
...
void testNormalConstraints(PBConfig config, PBFuzzer& fuzzer, int64_t rounds) {
  if (!printLess)
    cout << "testing normal constraints                                      "
         << endl;
  ...
  while (rounds < 0 ? true : (count < rounds)) {
    //     cout << "new round " << endl;
    count++;  // cout << "c rand() counts: " << RandomCounter::callCount << " "
              // <<endl;
    ...
    else if (constraint.getComparator() == GEQ)
      haveToResult = (sum >= constraint.getGeq());
    else
      haveToResult =
          (sum <= constraint.getLeq()) && (sum >= constraint.getGeq());
    ...
    // 	cout << "SAT / UNSAT = " << numSAT / (numSAT + numUNSAT) << " SAT " <<
    // numSAT  << " UNSAT " << numUNSAT << " total: " << (int64_t)numSAT +
    // (int64_t)numUNSAT << "     \r";
    //       cout << flush;
    //     }
  }
...
void testConditionalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                int64_t rounds) {
  if (!printLess)
    cout << "testing conditional constraints                                   "
            "   "
         << endl;
  ...
  while (rounds < 0 ? true : (count < rounds)) {
    ...
    for (int i = 0; i < conditionalCount; ++i) {
      conditionals.push_back((fuzzer.numer_of_variables + i + 1) *
                             (RandomCounter::rand() % 2 == 0 ? -1 : 1));
    }
    ...
    for (int32_t lit : conditionals) {
      conditionalIsTrue =
          conditionalIsTrue && (lit < 0 ? !assignment[-lit] : assignment[lit]);
    }
...
void testIncrementalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                int64_t rounds) {
  if (!printLess)
    cout << "testing incremental constraints                                   "
            "   "
         << endl;
  ...
  while (rounds < 0 ? true : (count < rounds)) {
    ...
    IncPBConstraint constraint(c.getWeightedLiterals(), PBLib::BOTH, c.getLeq(),
                               c.getGeq());

    if (c.getComparator() != BOTH) constraint.setComparator(c.getComparator());

    pb2cnf.encodeIncInital(constraint, formula, auxvars);

    if (RandomCounter::rand() % 100 + 1 <= 90) {
      if ((constraint.getComparator() == BOTH &&
           (RandomCounter::rand() % 100 + 1 <= 50)) ||
          (constraint.getComparator() == PBLib::GEQ)) {
        constraint.encodeNewGeq(
            constraint.getGeq() + (RandomCounter::rand() % 100), formula,
            auxvars);
      } else {
        constraint.encodeNewLeq(
            constraint.getLeq() - (RandomCounter::rand() % 100), formula,
            auxvars);
      }
    }
...
void testConditionalIncrementalConstraints(PBConfig config, PBFuzzer& fuzzer,
                                           int64_t rounds) {
  if (!printLess)
    cout << "testing incremental conditional constraints                       "
            "               "
         << endl;
