...
void PBFuzzer::writeToPBFile(PBConstraint opt_constraint,
                             vector<PBConstraint> constraints, int numVars,
                             string file) {
  ...
  for (int i = 0; i < constraints.size(); ++i) {
    ...
    if (constraint.getComparator() == BOTH &&
        constraint.getLeq() != constraint.getGeq()) {
      PBConstraint constraint = constraints[i];
  ...
  pbfile << "* #variable= " << numVars << " #constraint= " << numConstraints
         << endl;
  pbfile << "****************************************" << endl;
...
void PBFuzzer::scramble(vector<PBConstraint>& constraints) {
  ...
  for (int i = 0; i < constraints.size(); ++i) {
    ...
    for (int j = 0; j < rnd; ++j) {
      ...
      constraint.setGeq(constraint.getGeq() -
                        constraint.getWeightedLiterals()[index].weight);
      constraint.setLeq(constraint.getLeq() -
                        constraint.getWeightedLiterals()[index].weight);
...
PBConstraint PBFuzzer::generatePBProblem(
  ...
  while (constraints.size() < number_of_constraints + 1) {
    ...
    if (rnd <= pAMO) {
      ...
      for (int i = 0; i < constraintSize; ++i) {
        if (RandomCounter::rand() % 100 + 1 <= polarity_ratio)
          literals.push_back(
              WeightedLit(RandomCounter::rand() % numer_of_variables + 1, 1));
        else
          literals.push_back(WeightedLit(
              -(RandomCounter::rand() % numer_of_variables + 1), 1));
      }
      ...
      constraints.push_back(PBConstraint(
          literals, LEQ, RandomCounter::rand() % (constraintSize) + 2));
    } else {
      // PB
      for (int i = 0; i < constraintSize; ++i) {
        if (isBig) {
          if (RandomCounter::rand() % 100 + 1 <= polarity_ratio)
            literals.push_back(
                WeightedLit(RandomCounter::rand() % numer_of_variables + 1,
                            RandomCounter::rand() % max_big_weight_size + 1));
          else
            literals.push_back(
                WeightedLit(-(RandomCounter::rand() % numer_of_variables + 1),
                            RandomCounter::rand() % max_big_weight_size + 1));
          ...
          if (RandomCounter::rand() % 100 + 1 <= polarity_ratio)
            literals.push_back(
                WeightedLit(RandomCounter::rand() % numer_of_variables + 1,
                            RandomCounter::rand() % max_weight_size + 1));
          else
            literals.push_back(
                WeightedLit(-(RandomCounter::rand() % numer_of_variables + 1),
                            RandomCounter::rand() % max_weight_size + 1));
      ...
      if (isBig)
        constraints.push_back(PBConstraint(
            literals, LEQ,
            RandomCounter::rand() % (max_big_weight_size * constraintSize)));
      else
        constraints.push_back(PBConstraint(
            literals, LEQ,
            RandomCounter::rand() % (max_weight_size * constraintSize)));
    }
    formula.clearDatabase();
    SimplePBConstraint preEncodedConstraint =
        preEncoder.preEncodePBConstraint(constraints.back(), formula);
    if (preEncodedConstraint.getType() == DONTCARE) {
      if (formula.isSetToUnsat()) {
        retry = true;
        constraints.pop_back();
      } else {
        if (RandomCounter::rand() % 100 == 0 &&
            trivial_count <= max_trivial_constraints)
          trivial_count++;
        else {
          retry = true;
          constraints.pop_back();
    ...
    } else if (isAmk && preEncodedConstraint.getType() != AMK) {
      retry = true;
      constraints.pop_back();
    } else if (isAmo && preEncodedConstraint.getType() != AMO) {
      retry = true;
      constraints.pop_back();
    }
  }
