...
int main(int argc, char** argv) {
  ...
  if (parser.hasObjectiveFunction()) {
    ...
    assert(offset <= 0);
    if (offset < 0) {
      cout << "c warning: use offset " << offset << " to calculate real optimum"
           << endl;
    }

    cout << "p wcnf " << auxvars.getBiggestReturnedAuxVar() << " "
         << formula.getClauses().size() + parser.getObjConstraint().getN()
         << " " << top_weight << endl;
    ...
    cout << "c warning this is a decision instance, not a maxsat instance"
         << endl;
    cout << "p cnf " << auxvars.getBiggestReturnedAuxVar() << " "
  ...
  //   tend = clock();
  //   cout << "wall time: " << (tend - tstart) / CLOCKS_PER_SEC << " sec" <<
  //   endl;

  //   struct rusage rusage;
  //   getrusage( RUSAGE_SELF, &rusage );
  //   cout <<"Memory usage: "<< (size_t)(rusage.ru_maxrss / 1024L) << " mbyte"
  //   << endl;
